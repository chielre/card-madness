...
const isOfficialPack = () => {
    return props.pack.id.startsWith('cm_');
}
...
<template>
    <div class="madness-pack shrink-0  aspect-[0.65/1]  w-full select-none relative rounded-xl border-2 border-black outline-2 outline-white/50 -outline-offset-8 p-8 overflow-hidden text-left transition transform hover:-translate-y-1  active:scale-95  disabled:cursor-not-allowed " :class="selected ? 'scale-95 ring-8 ring-yellow-400/50' : 'group hover:scale-105 hover:-rotate-2 hover:z-20 hover:shadow-2xl '" :style="{ backgroundImage: `linear-gradient(to bottom, ${pack.backgroundColors.join(', ')})` }" :data-pack-id="pack.id" :data-selected="selected ? 'true' : 'false'">
        ...
        <button type="button" class="hover:bg-white hover:text-black transition-transform z-30 absolute outline-1 -outline-offset-4 top-3 right-3 w-8 h-8 rounded-full text-xl bg-black text-white flex items-center justify-center" :class="selected ? 'translate-x-0 translate-y-0' : 'group-hover:translate-x-0 group-hover:translate-y-0 delay-100 translate-x-20 -translate-y-20'" @click.stop="openInfo">
            <information-symbol />
        </button>
        ...
        <div class="relative text-white font-bold flex flex-col h-full items-center justify-between w-full ">
            ...
            <div v-if="pack.deprecated" class="group-hover:scale-0  transition duration-250 absolute bottom-0 left-0  right-0 z-10 bg-red-600 border-2 border-b-4 shadow-lg py-1 px-3 rotate-1 border-red-800 text-center rounded-xl">
                Leaving Soon
            </div>

            <div class="relative mt-8 group-hover:scale-110 transition duration-250">
                <img width="100%" class=" " :src="pack.logoUrl">
